    ...
    return (
        <center>
            <div>
                ...
                <button onClick={() => navigate(`/datasets/${datasetId}`)} style={{ color: "white" }}>
                    Back
                </button>
                <br/>
                ...
                {showCreateModal && (
                    <div className="modal">
                        <div className="modal-content">
                            ...
                            <h2>Create Training Instance</h2>
...
const DatasetTrain = () => {
    ...
    const handleSaveTraining = (e) => {
        e.preventDefault();
        const validKeywords = keywords.filter(kw => kw.trim().length > 0 && !kw.includes(' ') && !kw.includes(',') && kw.length <= 20);
        if (validKeywords.length === 0) {
            setError("Please enter at least one valid keyword.");
            return;
        }
